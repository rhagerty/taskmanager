...
const getAllEvents = async (user) => {
  ...
  try {
    ...
    console.log("Connected to DB TaskManager.");
    const db = client.db("TaskManager");

    const dbEvents = await db.collection("Events").find(query).toArray();
  ...
  client.close();
  console.log("Disconnected from DB TaskManager");
};
...
const addEvent = async (req, res) => {
  ...
  try {
    ...
    const result = await db.collection("Events").insertOne(eventObject);
